...
gulp.task('checkPrebuild', ['prebuild', 'copyPrebuildFiles'], function() {
    ...
    return deferedCheckPrebuild(function() {
        sendEmail({
            subject: 'build task has been successfully completed.',
...
function checkPrebuildFn(successFn, errorFn) {
    var countFileShellCMD = 'ls prebuild/ -l |grep "^-"|wc -l';
    ...
    if (totalFileNums == 1) {
        ...
        sendEmail({
            ...
            html: [
                '<p>Fail at:</p>',
                '<p>'+ countFileShellCMD +'</p>',
                '<p>Build task may fail at prebuild task, check your log carefully!</p>'
            ]
        });
        errorFn && errorFn();
    } else {
...
function delFile(index, fileList) {
    ...
    console.log('shell', delFileShellCMD);
...
function execTaskAfterPreBuild(callback) {
    ...
    console.log('toDelFileListMap', toDelFileListMap);
...
function sendEmail(EmailOptions, EmailConfig) {
    ...
    var mail = {
        ...
        to: 'nongjiawu@xinguangnet.com',
        subject: EmailOptions.subject || 'test',
        html: EmailOptions.html || 'mail test from gulp'
    };
