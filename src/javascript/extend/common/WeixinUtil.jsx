...
export default class WeixinUtil {
	...
	static genLink (pageName, data = {}) {
		let config = require('extend/config/config.json');
		let urlPrefix = config['pageOnline'];
		let linkUrlPrefix = `${urlPrefix}/${pageName}/index.html`;
		let targetUrl = linkUrlPrefix;

		for(let key in data)
			targetUrl = Util.appendParam4Url(targetUrl, key, data[key]);
	...
	static formatShareData(data, type = 'friendCircle') {
		let formatData = {
			'friends': {
				title: data.title,
				link: data.link,
