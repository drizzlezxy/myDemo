...
class PickerColumn extends Component {
  ...
  computeTranslate = (props) => {
    ...
    if (selectedIndex < 0) {
      ...
      this.onValueSelected(options[0], 0);
      selectedIndex = 0;
    }
    return {
  ...
  onValueSelected = (newValue, index) => {
    this.props.onChange(this.props.name, newValue, index);
  };
  ...
  handleTouchEnd = (event) => {
    ...
    setTimeout(() => {
      ...
      this.onValueSelected(options[activeIndex], activeIndex);
    }, 0);
  };
  ...
  handleItemClick = (option, index) => {
    if (option !== this.props.value) {
      this.onValueSelected(option, index);
    }
  };
    ...
    return options.map((option, index) => {
      ...
      return (
        <div
          ...
          className={className}
          style={style}
          onClick={() => this.handleItemClick(option,index)}>{option}</div>
      );
    });
  }
